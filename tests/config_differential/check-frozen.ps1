# Fails when a file frozen.tsv records has changed: the frozen legacy reader, a source it
# compiles, the oracle, or an input. The reader and its sources are what an old
# HeadTracking.ini is read through, so a change moves what a player's old file converts to;
# the oracle and the inputs are what the differential test holds that to.
